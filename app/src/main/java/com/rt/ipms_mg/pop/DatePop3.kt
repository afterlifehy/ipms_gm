...
import com.rt.ipms_mg.databinding.PopDate3Binding
 ...
 */
class DatePop3(val context: Context?, var startDate: String, var endDate: String, val resetType: Int, var callback: DateCallBack) :
    PopupWindow(context), View.OnClickListener {

    private lateinit var binding: PopDate3Binding
    ...
    private fun initView() {
        binding = PopDate3Binding.inflate(LayoutInflater.from(context))
        ...
        binding.cbTwoMonth.setOnClickListener(this)
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.cb_oneMonth -> {
                ...
                binding.cbTwoMonth.isChecked = false
            ...
            R.id.cb_twoMonth -> {
                ...
                binding.cbTwoMonth.isChecked = true
                binding.cbThreeMonth.isChecked = false
                val twoMonthLater  = LocalDate.now().plusMonths(2)
                startDate = TimeUtils.millis2String(System.currentTimeMillis(), "yyyy-MM-dd")
                endDate = twoMonthLater.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))
                binding.tvStartTime.text = startDate
                binding.tvEndTime.text = endDate
                startMillis = TimeUtils.string2Millis(startDate, "yyyy-MM-dd")
                endMillis = TimeUtils.string2Millis(endDate, "yyyy-MM-dd")
            ...
            R.id.cb_threeMonth -> {
                ...
                val threeMonthLater  = LocalDate.now().plusMonths(3)
                startDate = TimeUtils.millis2String(System.currentTimeMillis(), "yyyy-MM-dd")
                endDate = threeMonthLater.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))
                binding.tvStartTime.text = startDate
                binding.tvEndTime.text = endDate
                startMillis = TimeUtils.string2Millis(startDate, "yyyy-MM-dd")
                endMillis = TimeUtils.string2Millis(endDate, "yyyy-MM-dd")
